...
<table id="THIRD" lay-filter="THIRD"></table>
<form id="editForm" method="post" hidden="hidden" style="width: 200px">
    <div style="margin:15px 5px 10px 5px;color: #0C0C0C;width: 70px"> 三级指标：<input type="text" id="edit_thirdIndex"></div>
    <div style="margin:15px 5px 10px 5px;color: #0C0C0C;width: 70px"> 权重：<input type="text" height="30px" id="edit_thirdWeight"></div>
</form>
<!--添加指标-->
<form id="addForm" method="post" hidden="hidden" style="width: 200px;">
    <div style="margin:15px 5px 10px 5px;width: 70px"> 三级指标：<input type="text" id="name"></div>
    ...
    <div style="margin:15px 5px 10px 5px;width: 100px">
        所属二级指标：
        <select name="selectThirdIndex" id="parent" class="selectThirdIndex" style="margin-top: 5px">
...
<script>
    layui.use('table', function(){
        ...
        table.render({
            elem: '#THIRD'
            ,height: 'full-50'
            ,url: '/getThirdIndex' //数据接口
            ,page: true //开启分页
            ,cols: [[ //表头
                {field: 'tid', title: 'ID',templet: '#col_index',width:80, sort: true, fixed: 'left'}
                ,{field: 'tname', title: '三级指标'}
                ,{field: 'tweight', title: '权重',sort: true}
        ...
        //监听工具条
        table.on('tool(THIRD)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'edit'){ //编辑
                ...
                layer.open({
                    ...
                    success: function (layero, index) {
                        $("#edit_thirdIndex").attr("value", data.tname);
                        $("#edit_thirdWeight").attr("value", data.tweight);
                    },
                    btn1:function(index, layero){
                        var flag = 0;//记录指标名是否被修改，0是未修改，1是修改
                        if($("#edit_thirdIndex").val() != data.tname){
                            flag = 1;
                        }
                        var param = {"tid":data.tid,"thirdIndex":$("#edit_thirdIndex").val(),"thirdWeight":$("#edit_thirdWeight").val(),"flag":flag};
                        $.ajax({
                            type: "post",
                            url: "/updateWeight",
                ...
                layer.confirm("是否删除该指标？", {icon: 3, title: '提示'}, function (index) {
                    ...
                    $.ajax({
                        type: "post",
                        url: "/delIndex?level=3&id=" + data.sid,
                        dataType: "json",
                        contentType: "application/json",
                        success: function (result) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                for( var i = 0; i < result.length;i++){
                    ...
                    option.setAttribute("value",result[i].sid);// 给option的value添加值
                    option.innerText = result[i].sname;// 打印option对应的纯文本
